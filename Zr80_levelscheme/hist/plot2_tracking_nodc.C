{
  ...
  TPad *p;
  TCanvas *c = new TCanvas("c","Tracking_hist_nodc",700,600);
  c->cd();
  ...
  for(int i=0;i<nn;i++){
    if(i==0){
      h[nn] = (TH1F*)f[i]->Get("GRegamAB");
      h[nn]->SetName("Tracking_hist_nodc");
    }
    else
      h[nn]->Add((TH1F*)f[i]->Get("GRegamAB"));
    h[i] = (TH1F*)f[i]->Get("GRegamAB");
  }  
  h[nn]->Rebin(2);
  h[nn]->SetTitle("Traking_hist_nodc");
