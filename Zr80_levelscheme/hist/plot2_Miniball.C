{
  ...
  TPad *p;
  TCanvas *c = new TCanvas("c","Miniball_hist",700,600);
  c->cd();
  ...
  for(int i=0;i<nn;i++){
    if(i==0){
      h[nn] = (TH1F*)f[i]->Get("MBegamABdc");
      h[nn]->SetName("Miniball_hist");
    }
    else
      h[nn]->Add((TH1F*)f[i]->Get("MBegamABdc"));
    h[i] = (TH1F*)f[i]->Get("MBegamABdc");
  }  
  h[nn]->Rebin(2);
  h[nn]->SetTitle("Miniball_hist");
