{
  ...
  TPad *p;
  TCanvas *c = new TCanvas("c","Tracking_hist",700,600);
  c->cd();
  ...
  for(int i=0;i<nn;i++){
    if(i==0){
      h[nn] = (TH1F*)f[i]->Get("GRegamABdc");
      h[nn]->SetName("Tracking_hist");
    }
    else
      h[nn]->Add((TH1F*)f[i]->Get("GRegamABdc"));
    h[i] = (TH1F*)f[i]->Get("GRegamABdc");
  }  
  h[nn]->Rebin(2);
  h[nn]->SetTitle("Traking_hist");
