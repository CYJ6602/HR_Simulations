void plot_lowlevel_1n(const char* filename = "histos_1n.root")
{
  ...
  for(int i=0;i<nn;i++){
	...
	h2[nn][0] = (TH2F*)f[i]->Get("GRegamGRegamABdc");
	h2[nn][0]->SetName("Tracking_coin_ABdc");

	h2[nn][1] = (TH2F*)f[i]->Get("MBegamMBegamABdc");
	h2[nn][1]->SetName("Miniball_coin_ABdc");

	h2[nn][2] = (TH2F*)f[i]->Get("egamegamABdc");
	h2[nn][2]->SetName("Total_coin_ABdc");
      }
    else
      {
	...
	h2[nn][0]->Add((TH2F*)f[i]->Get("GRegamGRegamABdc"));
	h2[nn][1]->Add((TH2F*)f[i]->Get("MBegamMBegamABdc"));
	h2[nn][2]->Add((TH2F*)f[i]->Get("egamegamABdc"));
      }
